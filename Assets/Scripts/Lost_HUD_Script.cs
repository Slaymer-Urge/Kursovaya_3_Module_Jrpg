...
public class Lost_HUD_Script : MonoBehaviour
{
    ...
    public void Lose_Func()
    {
        ...
        Image_Char_1.sprite = Fight_Script_Empty.GetComponent<Fight_Script>().characters_list[0].gameObject.GetComponent<Sprite>();
        if (Fight_Script_Empty.GetComponent<Fight_Script>().characters_list[1] != null)
        {
            Image_Char_2.sprite = Fight_Script_Empty.GetComponent<Fight_Script>().characters_list[1].gameObject.GetComponent<Sprite>();
    ...
    void Update()
    {
        if (HUD_Script_Empty.active == true)
        {
            Image_Anim_Lose.transform.localScale = new Vector3(Image_Anim_Lose.transform.localScale.x + 0.008f, Image_Anim_Lose.transform.localScale.y, Image_Anim_Lose.transform.localScale.z);
            Debug.Log(Image_Anim_Lose.transform.localScale.x);
        }
    }
}
